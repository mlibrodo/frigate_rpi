#!/usr/bin/env python3
"""
Wildfire Defense System - Control Panel v3
...
def relay_all_off():
    """Force ALL relays off immediately — bypasses sequencer, safe for shutdown."""
    global _relay_state
    _relay_state = {i: False for i in range(1, 9)}
    if HAT_AVAILABLE:
        try:
            import lib8relind
            lib8relind.set_all(HAT_BOARD, 0)
            log.info("All relays forced OFF via set_all")
        except Exception as e:
            log.error(f"relay_all_off failed: {e}")
            # Fallback: turn off each relay individually
            try:
                import lib8relind
                for i in range(1, 9):
                    lib8relind.set(HAT_BOARD, i, 0)
            except Exception as e2:
                log.error(f"relay_all_off fallback failed: {e2}")
    else:
        log.info("All relays forced OFF (simulation)")
...
class EngineSequencer:
    ...
    def abort(self):
        """Immediately abort any running sequence and mark engine stopped."""
        self.running  = False
        self.starting = False
        self.stopping = False
    ...
    def _stop_seq(self, cb):
        ...
        try:
            ...
            relay_set(RELAY_IGNITION_KILL, False)  # Always release after kill

            self.running = False
            notify("Engine STOPPED")
        except Exception as e:
            notify(f"Stop FAILED: {e}")
            relay_set(RELAY_IGNITION_KILL, False)  # Safety: always release
        finally:
            self.stopping = False
...
def get_frigate_stats():
    try:
        ...
        with urllib.request.urlopen(
                f"{FRIGATE_URL}/api/events?limit=5&label=fire&label=smoke",
                timeout=2) as r:
            data = json.loads(r.read())
            return [
                f"{e.get('camera','?')} — {e.get('label','?')} "
                f"{int(e.get('score',0)*100)}%"
                for e in data
            ]
    except Exception:
        return []
...
class WildfirePanel(tk.Tk):
    ...
    def _build_ui(self):
        ...
        for key, label in tabs:
            btn = tk.Button(tabbar, text=label, font=FONT_TAB,
                            ...
                            relief="flat", bd=0, padx=0, pady=8, width=7,
    ...
    def _build_zones_tab(self):
        ...
        for ci, h in enumerate(["RELAY", "FUNCTION", "STATE"]):
    ...
    def _emergency_all_off(self):
        """Immediate all-off — aborts sequencer, forces all relays off via set_all."""
